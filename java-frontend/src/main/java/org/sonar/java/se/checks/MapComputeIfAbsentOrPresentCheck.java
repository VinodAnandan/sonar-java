...
@Rule(key = "S3824")
public class MapComputeIfAbsentOrPresentCheck extends SECheck {

  private static final MethodMatcher MAP_GET = mapMethod("get");
  private static final MethodMatcher MAP_PUT = mapMethod("put");

  private final Map<SymbolicValue, MapGetInvocation> mapGetInvocations = new HashMap<>();
  ...
  public ProgramState checkPreStatement(CheckerContext context, Tree syntaxNode) {
    if (syntaxNode.is(Tree.Kind.METHOD_INVOCATION)) {
      ...
      if (MAP_PUT.matches(mit)) {
        ...
        MapGetInvocation getOnSameMap = mapGetInvocations.get(mapSV);

        if (getOnSameMap != null && getOnSameMap.withSameKey(keySV)) {
